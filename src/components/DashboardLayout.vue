...
<template>
  <div class="min-h-screen bg-slate-50 flex text-slate-800">
    ...
    <aside
      :class="[
        'fixed inset-y-0 left-0 bg-slate-900 text-slate-200 z-30 flex flex-col justify-between transition-all duration-300 w-64 md:sticky',
        isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0 md:w-20'
      ]"
    >
      <div>
        <!-- Sidebar Header / Brand Logo -->
        <div class="h-16 flex items-center px-6 bg-slate-950/40 border-b border-slate-800 justify-between">
          <BrandLogo size="sm" :showText="isSidebarOpen" />
          <!-- Close button for mobile -->
          <button @click="toggleSidebar" class="p-1.5 rounded-lg hover:bg-slate-800 md:hidden text-slate-400">
            <X class="w-5 h-5" />
          </button>
        </div>

        <!-- Navigation Links -->
        <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto max-h-[calc(100vh-10rem)] scrollbar-thin">
          <template v-for="(item, index) in menuItems" :key="index">
            <!-- Header items -->
            <div v-if="item.header" class="pt-4 pb-1">
              <span v-if="isSidebarOpen" class="text-xs font-semibold text-slate-500 uppercase tracking-wider px-3">
                {{ item.header }}
              </span>
              <div v-else class="h-px bg-slate-800 my-2 mx-2"></div>
            </div>
            ...
            >
              ...
              >
                ...
                <span 
                  v-if="!isSidebarOpen" 
                  class="absolute left-16 bg-slate-950 text-white text-xs px-2.5 py-1.5 rounded-md opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-150 whitespace-nowrap shadow-lg border border-slate-800 z-50"
                >
                  {{ item.title }}
                </span>
              </span>
      ...
      <div class="p-4 bg-slate-950/40 border-t border-slate-800">
        ...
        <div v-else class="space-y-4">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 shadow-inner shrink-0">
          ...
          <button
            @click="handleLogout"
            class="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold bg-red-950/20 hover:bg-red-900/30 text-red-400 hover:text-red-300 border border-red-900/30 transition-all duration-150 cursor-pointer"
          >
